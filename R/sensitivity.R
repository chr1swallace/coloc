...
sensitivity.coloc <- function(obj,rule="H4 > 0.5",
    ...
    pass.init <- check(pp)
    message("Results ",if(check(pp)) { "pass" } else { "fail" }, " decision rule ",rule)

    testp12 <- 10^seq(log10(p1*p2),log10(min(p1,p1)),length.out=npoints)
    testH <- prior.snp2hyp(pp["nsnps"],p12=testp12,p1=p1,p2=p2)
    testpp <- as.data.frame(prior.adjust(pp,newp12=testp12,p1=p1,p2=p2,p12=p12))
    ...
    if(doplot) {
        H <- as.character(0:4)
        palette(viridis::viridis(5,alpha=1))
        par(mfrow=c(2,1))
    ...
    tck = -.01 # Reduce tick length
        ...
        m <- list(testH,testpp)
        ti <- list("Prior probabilities as a function of p12",
                   "Posterior probabilities as a function of p12")
        for(i in 1:2) {
        ...
        mtext(paste("Acceptance rule (shaded region):",rule.init),side=3,adj=1)
        }
    }
