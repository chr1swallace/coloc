...
##' @param calibrate the number of simulated samples to callibrate the posterior predictive p value
##' @param quiet suppress messages detailing progress
...
coloc.bma <- function(df1,df2,snps,response1="Y", response2="Y", family1="binomial", family2="binomial",
                     ...
                     plot.coeff=FALSE,r2.trim=0.95,callibrate=0,quiet=FALSE,callibrate.style=1,...) {
  snps <- unique(snps)
  n.orig <- length(snps)
  if(n.orig<2)
    return(1)
  ...
  ## callibrate?
  if(callibrate>0) {
    pcal <- matrix(NA,callibrate,length(wh))
    colnames(pcal) <- as.character(wh)
  }
  if(!quiet)
  ...
  ## callibrate?
  if(callibrate>0) {
    if(callibrate.style==1) { # set eta to best estimate
      theta.star <- rep(atan(ci$eta.mean), callibrate)
    }
    if(callibrate.style==2) { # sample eta from posterior
      theta.bma <- seq(0,pi,length=n.approx)[-c(1,n.approx)]
      theta.star <- sample(theta.bma, size=callibrate, prob=post[-c(1,n.approx)], replace=TRUE)
    }
    if(callibrate.style==3) { # sample eta from prior
      theta.bma <- seq(0,pi,length=n.approx)[-c(1,n.approx)]
      theta.star <- sample(theta.bma, size=callibrate, replace=TRUE)
    }
    
    for(i in wh) {
      b1 <- t(sapply(tan(theta.star), function(eta) {
        mvrnorm(n=1, mu=coef.2[[i]]/eta, Sigma=var.1[[i]]) }))
      b2 <- mvrnorm(n=callibrate, mu=coef.2[[i]], Sigma=var.2[[i]])
      ii <- as.character(i)
      for(j in 1:callibrate) {
        this.coloc <-  coloc.test.summary(b1[j,], b2[j,], var.1[[i]], var.2[[i]], plot.coeff=FALSE,bma=FALSE,bayes=FALSE,...)
        pcal[j,ii] <- pchisq(this.coloc@result["chisquare"], df=this.coloc@result["n"]-1, lower=FALSE)
      }
    }
  }
  ...
  if(callibrate==0) {
    ...
    ## callibrated thresholds
    pcal2 <- t(probs[wh]) %*% t(pcal) / sum(probs[wh])
    return(new("colocCallibrated",
               result=c(eta.hat=p[1],chisquare=NA,n=p[3]),
               ppp=stats[5],
               credible.interval=ci,
               bayes.factor=bf,
               cppp=c(p=stats["p"], cp=mean(pcal2 <= stats["p"]),
                 ppp=stats["ppp"], cppp=mean(pcal2 <= stats["ppp"]))))
  }
}
