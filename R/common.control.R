...
coloc.var.bma <- function(df1,snps=setdiff(colnames(df1),response),
    ...
    capture.output(lm1 <- multinom(f1, data=df1))
    ...
    for(i in wh) {
        ...
        if(!quiet)
            cat(".")
        capture.output(multiglm <- multinom(as.formula(paste("Y~",paste(modelsnps,collapse="+"))), data=df1))
        ...
        this.coloc <-coloc.var.test.summary(coef.1[[i]], coef.2[[i]], var[[i]], plot.coeff=FALSE,bma=TRUE,n.approx=n.approx,bayes.factor=bayes.factor,bayes=bayes)
        if(bayes) {
...
coloc.var.test.summary <- function(b1,b2,V,k=1,plot.coeff=TRUE,plots.extra=NULL,bayes=!is.null(bayes.factor),
  ...
  if(bayes) {
    ...
    mu <- function(theta) { M(theta) %*% (  cos(theta)*(W11%*%b1) +cos(theta)*(W12%*%b2)+sin(theta)*(W21%*%b1)+cos(theta)*(W22%*%b2) )}
    L <- function(theta) {
