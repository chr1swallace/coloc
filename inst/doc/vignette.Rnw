...
which $pp_{i1}<$ \texttt{thr} and $pp_{i2}<$ \texttt{thr}.  Models containing
\textbf{only} SNPs from this set will be ignored.  Note that models
containing one SNP from this set and one SNP \emph{not} in the set \textbf{will}
be evaluated.
...
TODO
\section*{Configuration analyses}
\label{sec-4}
...
\begin{itemize}
\item H$_{\text{0}}$: neither trait has a genetic association in the region
\item H$_{\text{1}}$: only trait 1 has a genetic association in the region
\item H$_{\text{2}}$: only trait 2 has a genetic association in the region
\item H$_{\text{3}}$: both traits are associated, but with different causal variants
\item H$_{\text{4}}$: both traits are associated and share a single causal variant
\end{itemize}

The function \texttt{coloc.config} is ideally suited to the case when only summary data are
available, when a \texttt{data.frame} containing three columns giving the p
...
the wrapper function \texttt{coloc.config.datasets()} for the analysis.
