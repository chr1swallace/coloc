...
Each dataset must represent the genetic association statistics for *a single trait* in *a single genomic region*. This means you should not, for example,
* supply coloc with a whole genome or whole chromosome of test statistics
...
* represent a dense coverage of the region (so don't threshold on p value)
...
It is useful to plot the pair of datasets, and check that the shared snps cover any peaks in either dataset. 

```{r}
plot_datasets(minimum_data, minimum_ccdata)
```

Here they will all be shared because they are copies of the same dataset. But any snps not coloured blue would be unique to one dataset or the other.
