...
We can run SuSiE to see how many signals it finds.
...
finemap.susie(s2)
```
SuSiE can be computationally expensive in large regions, so it is good practice to run it once  the susie_rss function in the [susieR package](https://stephenslab.github.io/susieR/) that automates running until convergence and saves a little extra information about snp names to make subsequent coloc processing simpler.  finemap.susie() just extracts the credible sets SuSiE found and prints at most the top 5 SNPs in each.  Here, it does indeed find two signals for dataset 1 (the 75th SNP named s25.1 and the 25th SNP named s25).
