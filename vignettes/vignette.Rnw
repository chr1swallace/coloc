...
\subsection*{The basic coloc.abf function}
The function \texttt{coloc.abf} is ideally suited to the case when only summary data are
...
supplied. A wrapper function is avalailable to run all these steps but we will first generate the p values manually to give all the details.
...
X1 <- new("SnpMatrix",as.matrix(data@df1[,2:11]))
X2 <- new("SnpMatrix",as.matrix(data@df2[,2:11]))
...
Here, we have simulated full genotype data, so I will use
the wrapper function \texttt{coloc.abf.datasets()} for the analysis.
